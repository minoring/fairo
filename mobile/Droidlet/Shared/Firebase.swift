...
class Droidbase {
    ...
    func getObjectAPI(completion: ((UIImage) -> ())?) {
        ...
        var request = URLRequest(url: URL(string: "https://yss21031-default-rtdb.firebaseio.com/human-object-recognition.json")!,timeoutInterval: Double.infinity)
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                guard let dicts = try JSONSerialization.jsonObject(with: data, options: []) as? [String : Any],
                      let dict = dicts.first?.value as? [String : Any], let snapshot = dict["snapshot"] as? String else {
                    return
                }
    ...
    func getImageURL(name: String, completion: ((URL) -> ())?) {
        let starsRef = storageRef.child("images/\(name).png")
